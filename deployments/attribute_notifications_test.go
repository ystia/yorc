...
func TestBuildAttributeData(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := BuildAttributeDataFromPath(context.Background(), tt.args.outputPath)
			if err != nil {
...
func assertOneConsulKeyExistAndHasValueInList(t *testing.T, keyPrefix string, expectedValue []byte) {
...
func testAddSubstitutionMappingAttributeHostNotification(t *testing.T, deploymentID string) {
	...
	assertOneConsulKeyExistAndHasValueInList(t, path.Join(instancesPath, "Compute/0/attribute_notifications/private_address/"), []byte("SrvImpl1Instance/0/attributes/a3"))

	// a1 of SrvImpl1Instance need to be notified of attr1 due to {get_attribute: [SELF, attr1]}
	assertOneConsulKeyExistAndHasValueInList(t, path.Join(instancesPath, "SrvImpl1Instance/0/attribute_notifications/attr1/"), []byte("SrvImpl1Instance/0/attributes/a1"))

	// a2 of SrvImpl1Instance need to be notified of cap1_attr1 due to {get_attribute: [SELF, cap1, cap1_attr1]}
	assertOneConsulKeyExistAndHasValueInList(t, path.Join(instancesPath, "SrvImpl1Instance/0/capabilities/cap1/attribute_notifications/cap1_attr1/"), []byte("SrvImpl1Instance/0/attributes/a2"))
	// capabilities.cap1.cap1_attr1 of SrvImpl1Instance need to be notified of cap1_attr1 due to substition mapping
	assertOneConsulKeyExistAndHasValueInList(t, path.Join(instancesPath, "SrvImpl1Instance/0/capabilities/cap1/attribute_notifications/cap1_attr1/"), []byte("SrvImpl1Instance/0/attributes/capabilities.cap1.cap1_attr1"))
	// capabilities.cap1.cap1_attr1 of SrvImpl2Instance need to be notified of cap1_attr1 due to substition mapping
	assertOneConsulKeyExistAndHasValueInList(t, path.Join(instancesPath, "SrvImpl1Instance/0/capabilities/cap1/attribute_notifications/cap1_attr1/"), []byte("SrvImpl2Instance/0/attributes/capabilities.cap1.cap1_attr1"))

	// capabilities.cap1.cap1_attr1 of SrvImpl2Instance need to be notified of cap1_attr1 due to substition mapping
	assertOneConsulKeyExistAndHasValueInList(t, path.Join(instancesPath, "SrvImpl2Instance/0/capabilities/cap1/attribute_notifications/cap1_attr1/"), []byte("SrvImpl2Instance/0/attributes/capabilities.cap1.cap1_attr1"))
