...
func testDeleteInstance(t *testing.T) {
	...
	actualState, err := GetInstanceStateString(ctx, deploymentID, nodeName, instanceName1)
	require.NoError(t, err)
	require.Equal(t, state, actualState)
	actualState, err = GetInstanceStateString(ctx, deploymentID, nodeName, instanceName11)
	require.NoError(t, err)
	require.Equal(t, state, actualState)

	err = DeleteInstance(ctx, deploymentID, nodeName, instanceName1)
...
func testDeleteAllInstances(t *testing.T) {
	...
	actualState, err := GetInstanceStateString(ctx, deploymentID, nodeName1, instanceName1)
	require.NoError(t, err)
	require.Equal(t, state, actualState)
	actualState, err = GetInstanceStateString(ctx, deploymentID, nodeName1, instanceName11)
	require.NoError(t, err)
	require.Equal(t, state, actualState)
	actualState, err = GetInstanceStateString(ctx, deploymentID, nodeName11, instanceName1)
	require.NoError(t, err)
	require.Equal(t, state, actualState)
	actualState, err = GetInstanceStateString(ctx, deploymentID, nodeName11, instanceName11)
	require.NoError(t, err)
	require.Equal(t, state, actualState)

	err = DeleteAllInstances(ctx, deploymentID, nodeName1)
