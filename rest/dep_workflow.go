...
func (s *Server) newWorkflowHandler(w http.ResponseWriter, r *http.Request) {
	...
	dExits, err := deployments.DoesDeploymentExists(deploymentID)
	if err != nil {
		log.Panicf("%v", err)
	}
	if !dExits {
	...
	deploymentStatus, err := deployments.GetDeploymentStatus(deploymentID)
	if err != nil {
		log.Panicf("%v", err)
	}
	...
	workflows, err := deployments.GetWorkflows(deploymentID)
	if err != nil {
		log.Panic(err)
	}
	...
	if len(body) > 0 {
		...
		for _, nodeInstances := range wfRequest.NodesInstances {
			...
			nodeExists, err := deployments.DoesNodeExist(deploymentID, nodeName)
			if err != nil {
				log.Panicf("%v", err)
			}
			if !nodeExists {
...
func (s *Server) getWorkflowHandler(w http.ResponseWriter, r *http.Request) {
	...
	wfSteps, err := deployments.ReadWorkflow(deploymentID, workflowName)
	if err != nil {
		log.Panic(err)
	}
