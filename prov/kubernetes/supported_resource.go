...
func (yorcDep *yorcK8sDeployment) setAttributes(ctx context.Context, e *execution) (err error) {
	return deployments.SetAttributeForAllInstances(e.deploymentID, e.nodeName, "replicas", fmt.Sprint(*yorcDep.Spec.Replicas))
}
...
func (yorcSts *yorcK8sStatefulSet) setAttributes(ctx context.Context, e *execution) error {
	return deployments.SetAttributeForAllInstances(e.deploymentID, e.nodeName, "replicas", fmt.Sprint(*yorcSts.Spec.Replicas))
}
...
func (yorcSvc *yorcK8sService) setAttributes(ctx context.Context, e *execution) (err error) {
	...
	for _, val := range yorcSvc.Spec.Ports {
		if val.NodePort != 0 {
			...
			err = deployments.SetAttributeForAllInstances(e.deploymentID, e.nodeName, "k8s_service_url", str)
			if err != nil {
				return errors.Wrap(err, "Failed to set attribute")
			}
			err = deployments.SetAttributeForAllInstances(e.deploymentID, e.nodeName, "node_port", strconv.Itoa(int(val.NodePort)))
			if err != nil {
				return errors.Wrap(err, "Failed to set attribute")
			}
		}
