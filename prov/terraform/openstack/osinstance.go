	...
	if storageKeys, err := deployments.GetRequirementsKeysByNameForNode(g.kv, deploymentId, nodeName, "local_storage"); err != nil {
		...
		for _, storagePrefix := range storageKeys {
			...
			if volumeNodeName, err := g.getStringFormConsul(storagePrefix, "node"); err != nil {
				...
				if kp, _, _ := g.kv.Get(path.Join(deployments.DeploymentKVPrefix, deploymentId, "topology/instances", volumeNodeName, "properties/volume_id"), nil); kp != nil {
					...
					go func() {
						for {
							...
							if kp, _, _ := g.kv.Get(path.Join(deployments.DeploymentKVPrefix, deploymentId, "topology/instances", volumeNodeName, "attributes/volume_id"), nil); kp != nil {
								if dId := string(kp.Value); dId != "" {
									resultChan <- dId
									return
								}
