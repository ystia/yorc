...
func (c *Check) notify() {
	...
	if c.Report.Status == CheckStatusPASSING {
		...
		events.WithContextOptionalFields(c.ctx).NewLogEntry(events.INFO, c.Report.DeploymentID).Registerf("Check monitoring is back to normal for node (%s-%s)", c.Report.NodeName, c.Report.Instance)

	} else if c.Report.Status == CheckStatusCRITICAL {
		// Node in ERROR
		nodeState = tosca.NodeStateError
		events.WithContextOptionalFields(c.ctx).NewLogEntry(events.ERROR, c.Report.DeploymentID).Registerf("Check monitoring returned a connection failure for node (%s-%s)", c.Report.NodeName, c.Report.Instance)
	}
