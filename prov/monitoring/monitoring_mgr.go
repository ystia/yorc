...
func init() {
	dispatcher.RegisterPreActivityHook(removeMonitoringHook)
	dispatcher.RegisterPostActivityHook(addMonitoringHook)
}
...
func addMonitoringHook(ctx context.Context, cfg config.Configuration, taskID, deploymentID, target string, activity dispatcher.Activity) {
	...
	switch {
	case activity.Type() == dispatcher.ActivityTypeDelegate && strings.ToLower(activity.Value()) == "install",
		activity.Type() == dispatcher.ActivityTypeSetState && activity.Value() == tosca.NodeStateStarted.String():
...
func removeMonitoringHook(ctx context.Context, cfg config.Configuration, taskID, deploymentID, target string, activity dispatcher.Activity) {
	...
	switch {
	case activity.Type() == dispatcher.ActivityTypeDelegate && strings.ToLower(activity.Value()) == "uninstall",
		activity.Type() == dispatcher.ActivityTypeSetState && activity.Value() == tosca.NodeStateDeleted.String():
