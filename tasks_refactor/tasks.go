...
import (
	...
	"github.com/ystia/yorc/tasks_refactor/workflow"
)
...
func GetTaskRelatedSteps(kv *api.KV, taskID string) ([]workflow.Step, error) {
	...
	for _, kvp := range kvps {
		steps = append(steps, workflow.Step{Name: path.Base(kvp.Key), Status: string(kvp.Value)})
	}
	return steps, nil
}

// TaskStepExists checks if a task step exists with a stepID and related to a given taskID and returns it
func TaskStepExists(kv *api.KV, taskID, stepID string) (bool, *workflow.Step, error) {
	...
	return true, &workflow.Step{Name: stepID, Status: string(kvp.Value)}, nil
}

// UpdateTaskStepStatus allows to update the task step status
func UpdateTaskStepStatus(kv *api.KV, taskID string, step *workflow.Step) error {
...
func CheckTaskStepStatusChange(before, after string) (bool, error) {
	...
	stBefore, err := workflow.ParseStepStatus(before)
	if err != nil {
		return false, err
	}
	stAfter, err := workflow.ParseStepStatus(after)
	if err != nil {
		return false, err
	}

	if stBefore != workflow.StepStatusERROR || stAfter != workflow.StepStatusDONE {
		return false, nil
	}
	return true, nil
}
